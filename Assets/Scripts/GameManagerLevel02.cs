...
public class GameManagerLevel02 : MonoBehaviour
{
    ...
    public float levelTwoTimeLeft = 240.0f;

    private float lightTimer = 25.0f;
    ...
    public void Start()
    {
        ...
        sirenIndex = UnityEngine.Random.Range(90.0f, 35.0f);
        ...
        counter = PlayerPrefs.GetInt("counterLevelTwo");
    ...
    public void Update()
    {
        if (Math.Round(levelTwoTimeLeft, 0) == Math.Round(sirenIndex, 0))
        {
        ...
        if (Math.Round(levelTwoTimeLeft, 0) == Math.Round(lightIndex, 0))
        {
        ...
        if (levelTwoStrikes == 2)
        {
            levelTwoTimeLeft = 0.0f;
            EndGame();
        }
        if (LevelCompleted() == true)
        {
            ...
            {
                finalTime = levelTwoTimeLeft;
                PlayerPrefs.SetFloat("levelTwoFinalTime", finalTime);
                PlayerPrefs.Save();
            }
        }
        levelTwoTimeLeft -= Time.deltaTime;

        if (levelTwoTimeLeft <= 0)
        {
            levelTwoTimeLeft = 0;
        }

        if (levelTwoTimeLeft <= 0.0f && levelIsPassed == false)
        {
            levelTwoTimeLeft = 0.0f;
            EndGame();
        }

        if (levelTwoTimeLeft > 0.0f && levelIsPassed == true)
        {
            levelTwoTimeLeft = 0.0f;
            PassedLevel();
        }
        ...
        {
            levelTwoTimeLeft = 1.0f;
        }

        PlayerPrefs.SetString("levelIsPassed2", levelIsPassed.ToString());
        PlayerPrefs.Save();
    }

    public bool LevelCompleted()
    {
        if (Wires() == true && Symbols() == true && Memory() == true && ComplexWires() == true && SimonSays() == true)
        {
            return true;
        }
        else
    ...
    public void PassedLevel()
    {
        if (levelIsPassed == true)
        {
            Cursor.SetCursor(null, Vector2.zero, CursorMode.Auto);
            counter++;
            PlayerPrefs.SetInt("counterLevelTwo", counter);
            PlayerPrefs.Save();
            if (!secondCamera.activeSelf)
            {
                bomb.SetActive(false);
