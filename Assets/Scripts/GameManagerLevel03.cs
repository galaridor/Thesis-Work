...
public class GameManagerLevel03 : MonoBehaviour
{
    ...
    public float levelThreeTimeLeft = 180.0f;

    private float lightTimer = 30.0f;
    ...
    public int levelThreeStrikes = 0;
    ...
    public bool simonSaysFinal = false;
    public bool morseCodeFinal = false;
    ...
    public void Start()
    {
        ...
        sirenIndex = UnityEngine.Random.Range(90.0f, 40.0f);
        ...
        counter = PlayerPrefs.GetInt("counterLevelThree");
    ...
    public void Update()
    {
        if (Math.Round(levelThreeTimeLeft, 0) == Math.Round(sirenIndex, 0))
        {
        ...
        if (Math.Round(levelThreeTimeLeft, 0) == Math.Round(lightIndex, 0))
        {
        ...
        if (levelThreeStrikes == 1)
        {
            levelThreeTimeLeft = 0.0f;
            EndGame();
        }
        if (LevelCompleted() == true)
        {
            ...
            {
                finalTime = levelThreeTimeLeft;
                PlayerPrefs.SetFloat("levelThreeFinalTime", finalTime);
                PlayerPrefs.Save();
            }
        }
        levelThreeTimeLeft -= Time.deltaTime;

        if (levelThreeTimeLeft <= 0)
        {
            levelThreeTimeLeft = 0;
        }

        if (levelThreeTimeLeft <= 0.0f && levelIsPassed == false)
        {
            levelThreeTimeLeft = 0.0f;
            EndGame();
        }

        if (levelThreeTimeLeft > 0.0f && levelIsPassed == true)
        {
            levelThreeTimeLeft = 0.0f;
            PassedLevel();
        }
        ...
        {
            levelThreeTimeLeft = 1.0f;
        }

        PlayerPrefs.SetString("levelIsPassed3", levelIsPassed.ToString());
        PlayerPrefs.Save();
    }

    public bool LevelCompleted()
    {
        if (Wires() == true && Symbols() == true && Memory() == true && ComplexWires() == true && SimonSays() == true && MorseCode() == true)
        {
            return true;
        }
        else
    ...
    public bool MorseCode()
    {
        return morseCodeFinal;
    }
    ...
    public void PassedLevel()
    {
        if (levelIsPassed == true)
        {
            Cursor.SetCursor(null, Vector2.zero, CursorMode.Auto);
            counter++;
            PlayerPrefs.SetInt("counterLevelThree", counter);
            PlayerPrefs.Save();
            if (!secondCamera.activeSelf)
            {
                bomb.SetActive(false);
